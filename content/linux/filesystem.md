...
Antes de Windows, Linux y los otros sistemas modernos que conocemos hoy en día;
existía DOS (Disk Operating System). Era únicamente CLI, sin GUIs. Sin embargo,
podías instalar _Windows_ por encima de DOS.

Para las unidades de memoria se utilizan:

- `A` y `B` para los _floppy disks_,
- `C` para el disco duro,
- y etc, dependiendo de cuantas unidades tengas instaladas en el equipo.

En DOS podías instalar tus programas donde te diese la gana. _Windows_, sin
...
# Sistema
## `boot`
Aquí se guardan los datos que necesita el sistema operativo para encenderse
(_boot_). Por este motivo, no es recomendable toquetear los archivos de este
directorio.
...
Los drives, el kernel y determinadas aplicaciones acuden a este directorio para
...
archivos (ver [archivos](@/linux/files.md)).
...
> Si quieres montar algo manualmente, déjalo en `/mnt/`
